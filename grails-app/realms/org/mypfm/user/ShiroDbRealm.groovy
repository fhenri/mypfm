...
class ShiroDbRealm {
    ...
    def authenticate(authToken) {
        ...
        // exception.
        def user = org.mypfm.user.ShiroUser.findByUsername(username)
        if (!user) {
        ...
        // in the database.
        def account = new SimpleAccount(username, user.passwordHash, "org.mypfm.user.ShiroDbRealm")
    ...
    def hasRole(principal, roleName) {
        def roles = org.mypfm.user.ShiroUser.withCriteria {
            roles {
                eq("name", roleName)
            }
    ...
    def hasAllRoles(principal, roles) {
        def r = org.mypfm.user.ShiroUser.withCriteria {
            roles {
                'in'("name", roles)
            }
            eq("username", principal)
    ...
    def isPermitted(principal, requiredPermission) {
        ...
        def user = org.mypfm.user.ShiroUser.findByUsername(principal)
        ...
        def results = org.mypfm.user.ShiroUser.executeQuery("select distinct p from ShiroUser as user join user.roles as role join role.permissions as p where user.username = '$principal'")
